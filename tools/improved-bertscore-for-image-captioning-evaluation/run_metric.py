...
if __name__ == '__main__':
    ...
    parser.add_argument('--samples_fn', type=str, default='example/flickr.json')
    parser.add_argument('--output_name', type=str, default='flickr')
    ...
    with open(args.samples_fn, 'r') as file:
        samples = json.load(file)
    samples = compute_hiddens_and_idf(samples, args.output_name, save=False)
    idf_dict = get_idf_dict_from_samples(samples)
                ...
                for next_idx in range(len(samples[str(i)]['refs'])):
                    if next_idx != curr_idx:
                        R_masks = matrix_search_mismatch(samples[str(i)]['refs_hid'][next_idx],
                                                         ...
                                                         THRESHOLD=args.threshold, idfs=samples[str(i)]['refs_idf'][next_idx])
                        current_record[next_idx] = R_masks
                samples[str(i)]['mismatch'].append(current_record)
        output_name = args.output_name + '.'
        torch.save(samples, os.path.join(args.output_path, output_name + 'samples.pt'))

    print('---> computing scores')
    for i in trange(len(samples)):
        tokens_cand = tokenizer.tokenize(samples[str(i)]['cand'][0])
        idfs_cand = collate_idf_tokenized([tokens_cand], tokenizer.convert_tokens_to_ids, idf_dict).reshape(-1)
        R_full = 0.
                ...
                if left_idx != next_idx:
                    ...
                    for k in range(len(marks_base)):
                        if marks_base[k] == 0:
                            ...
                            if not duplicate:
                                hidden_ref = torch.cat([hidden_ref, samples[str(i)]['refs_hid'][left_idx][0][k].reshape(1, 1, -1)], 1)
                                tokens_ref.append(tokens_left[k])

            idfs_ref = collate_idf_tokenized([tokens_ref], tokenizer.convert_tokens_to_ids,
                                             idf_dict).reshape(-1)

            score = matrix_compute_full(hidden_ref, samples[str(i)]['cand_hid'][0], args.threshold, idfs_ref)

            score_rm = matrix_compute_rm_stop_word(hidden_ref, samples[str(i)]['cand_hid'][0], args.threshold, tokens_ref, tokens_cand)
