...
class HeldoutCOCO(PairedDataset):  # TODO: we have val data from train2014 in Heldout datasets
    def __init__(self, image_field, text_field, img_root, ann_root, id_root):
        """
        Read data and split it into data sets
        :param image_field:
        :param text_field:
        :param img_root: directory of M2 images
        :param ann_root: directory of M2 captions
        :param id_root: directory of Syncap data (with dataset_splits.json, encoded_captions.json, word_map.json)
        """
        roots = {}
        roots['train'] = {
            'img': os.path.join(img_root, 'train2014'),
        ...
        ids = {}
        d = json.load(open(os.path.join(id_root, 'dataset_splits.json')))
        ids['train'] = set([int(i) for i in d['train_images']])
        ids['val'] = set([int(i) for i in d['val_images']])
        ids['test'] = set([int(i) for i in d['test_images']])

        id2captions = json.load(open(os.path.join(id_root, 'captions.json')))

        with nostdout():
            self.train_examples, self.val_examples, self.test_examples = self.get_samples(roots, ids, id2captions)
        examples = self.train_examples + self.val_examples + self.test_examples
        super(HeldoutCOCO, self).__init__(examples, {'image': image_field, 'text': text_field})
        ...
        for split in ['train', 'val', 'test']:
            ...
            for index in range(len(ids)):
                ...
                if img_id in img_ids and imgid2count[img_id] < 5:
                    caption = imgid2captions[str(img_id)][imgid2count[img_id]]  # one caption of that image (preprocessed)
