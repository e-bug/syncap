...
class BUTRModel(CaptioningEncoderDecoderModel):
    def __init__(self, args):
        super(BUTRModel, self).__init__()
    ...
    def add_args(parser):
        ...
        group.add_argument("--decoder-learning-rate", default=1e-4)
...
class TopDownRankingDecoder(CaptioningDecoder):
    ...
    def __init__(self, word_map, embed_dim=300, encoder_output_dim=2048,
        ...
        self.fc_w = nn.Linear(language_generation_lstm_dim, self.vocab_size, bias=True)
        self.fc_t = nn.Linear(language_generation_lstm_dim, self.vocab_size, bias=True)
    ...
    def embed_captions(self, captions, decode_lengths):
        ...
        lang_enc_hidden_activations = torch.zeros((batch_size, self.language_encoding_lstm_size), device=device)
        ...
        for t in range(max(decode_lengths)):
            ...
            lang_enc_hidden_activations[decode_lengths == t + 1] = h_lan_enc[decode_lengths == t + 1]
    ...
    def forward_multi_step(self, encoder_output, prev_word_embeddings, states):
        ...
        v_hat = self.attention(images_embedded, h_lan_enc)
        h_lan_gen, c_lan_gen = self.language_generation_lstm(h_lan_gen, c_lan_gen, h_lan_enc, v_hat)
        w_scores = self.fc_w(self.dropout(h_lan_gen))
        t_scores = self.fc_t(self.dropout(h_lan_gen))
